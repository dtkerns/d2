...
This paper describes an open-source tool for identifying and generating DSAs given a set of input workloads. We describe how it can be used to identify good candidates for acceleration at the SB granularity in a resource-constrained target, no matter what domain the application lives in. The **D2** tool provides an exhaustive list of SBs that can be simulated to determine the optimal size.
